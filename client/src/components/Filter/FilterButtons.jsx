...
const FilterButtons = ({filterData, flag}) => {
  ...
  return (
    <div
      className="border border-gray-300 rounded-md px-4 py-1 relative
    cursor-pointer hover:border-gray-400"
    >
      <p
        className="text-base tracking-wide text-card flex items-center gap-2"
      ...
      >
        ...
        <IoChevronDown
          className={`text-base text-card duration-150 transition-all ease-in-out 
  ${filterMenu ? "rotate-180" : "rotate-0"}`}
      ...
      {filterData && filterMenu && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 50 }}
          className="w-40 z-50 backdrop-blur-sm max-h-44 overflow-y-scroll py-2 flex flex-col rounded-md shadow-md absolute top-8 left-0"
        >
          {filterData?.map((data) => (
            <div
              key={data.name}
              className="flex items-center justify-center gap-2 py-1 hover:bg-gray-200 ml-4"
